...
/*$("input").keyup(function(){
	var $parent = $(this).parent();
	$parent.next().html(/<script.*>/.test($(this).val()) ? $parent.prev().text().replace(/：/, "") + "不能输入非法内容" : "");
});*/
...
// 小括号里使用?:取消引用，对不必要的小括号引用加上?:可以提高正则表达式的运算效率
...
// replace方法中第一个参数如果是正则，有存在分组，则第二个参数可以通过$n使用对应的分组比如$1找到第一个小括号里匹配到的内容
...
// 匹配标签
console.log("<h1>233</h1>".match(/^<([a-z]+[a-z\d]*)[^>]*>.*<\/\1>$/));

console.log("asdfasdf>123".match(/[^>]*/));

// 匹配邮箱
// console.log("102020202@qq.com".match(/^([a-zA-Z\d])\.?\1@[a-z\d]+\.(?:com|cn|org|edu)$/));
